...
from kgfs.config import HnswSettings, KGFSConfig, SemanticSettings, VectorSettings
...
from kgfs.search.backends.hnsw import HnswVectorBackend
...
def _context(tmp_path: Path) -> SearchContext:
    ...
    config = KGFSConfig(
        ...
        vectors=VectorSettings(backend="hnsw", hnsw=HnswSettings(enabled=True)),
    )
    return SearchContext(conn=conn, config=config)


def test_hnsw_rebuild_missing_dependency_is_helpful(tmp_path: Path) -> None:
    if importlib.util.find_spec("hnswlib") is not None and importlib.util.find_spec("numpy") is not None:
        pytest.skip("hnswlib is installed; installed-backend tests cover rebuild behavior.")

    with pytest.raises(RuntimeError, match="hnswlib"):
        HnswVectorBackend().rebuild(_context(tmp_path), model_name="fake-local-model")


@pytest.mark.skipif(
    importlib.util.find_spec("hnswlib") is None or importlib.util.find_spec("numpy") is None,
    reason="hnswlib/numpy optional dependencies are not installed",
)
def test_hnsw_rebuild_search_and_clear_when_dependency_installed(tmp_path: Path) -> None:
    context, source = _indexed_context(tmp_path)
    before_text = source.read_text(encoding="utf-8")
    backend = HnswVectorBackend()
    ...
    assert rebuild.artifact_path is not None and rebuild.artifact_path.exists()
...
def _indexed_context(tmp_path: Path) -> tuple[SearchContext, Path]:
    ...
    source.write_text("motor torque hnsw notes", encoding="utf-8")
    (root / "circuits.md").write_text("op amp hnsw notes", encoding="utf-8")
    conn = connect_database(tmp_path / "kgfs.sqlite3")
    initialize_database(conn)
    config = KGFSConfig(
