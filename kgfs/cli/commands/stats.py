...
def stats(
    ...
    table.add_row("OCR enabled", str(config.ocr.enabled))
    table.add_row("OCR indexed files", str(stats_data["ocr_indexed_files"]))
    table.add_row("OCR failures", str(stats_data["ocr_failures"]))
    table.add_row("OCR cache entries", str(stats_data["ocr_cache_entries"]))
    table.add_row("OCR cache text size", format_bytes(stats_data["ocr_cache_text_bytes"]))
    table.add_row("Media enabled", str(config.media.enabled))
    table.add_row("Media metadata rows", str(stats_data["media_metadata_count"]))
    table.add_row("Media text rows", str(stats_data["media_text_count"]))
    table.add_row("Media embeddings", str(stats_data["media_embedding_count"]))
    table.add_row("Media derived text size", format_bytes(stats_data["media_text_bytes"]))
