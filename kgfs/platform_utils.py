...
def risk_warning(path: Path) -> str:
    """Warn when a configured folder looks broader than intended."""

    expanded = Path(path).expanduser()
    if expanded.parent == expanded:
        return "Root folder; avoid unless intentional"

    raw = str(path)
    if platform.system() == "Windows" and re.fullmatch(r"[A-Za-z]:[\\/]*", raw):
        return "Drive root; avoid unless intentional"
