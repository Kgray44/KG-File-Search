...
from kgfs.config import FaissSettings, KGFSConfig, SemanticSettings, VectorSettings
...
from kgfs.search.backends.faiss import FaissVectorBackend
...
def _context(tmp_path: Path) -> SearchContext:
    ...
    config = KGFSConfig(
        ...
        vectors=VectorSettings(backend="faiss", faiss=FaissSettings(enabled=True)),
    )
    return SearchContext(conn=conn, config=config)


def test_faiss_rebuild_missing_dependency_is_helpful(tmp_path: Path) -> None:
    if importlib.util.find_spec("faiss") is not None and importlib.util.find_spec("numpy") is not None:
        pytest.skip("FAISS is installed; installed-backend tests cover rebuild behavior.")

    with pytest.raises(RuntimeError, match="faiss-cpu"):
        FaissVectorBackend().rebuild(_context(tmp_path), model_name="fake-local-model")


@pytest.mark.skipif(
    importlib.util.find_spec("faiss") is None or importlib.util.find_spec("numpy") is None,
    reason="FAISS/numpy optional dependencies are not installed",
)
def test_faiss_rebuild_search_and_clear_when_dependency_installed(tmp_path: Path) -> None:
...
def _indexed_context(tmp_path: Path) -> tuple[SearchContext, Path]:
    ...
    source.write_text("motor torque faiss notes", encoding="utf-8")
    (root / "circuits.md").write_text("op amp faiss notes", encoding="utf-8")
    conn = connect_database(tmp_path / "kgfs.sqlite3")
    initialize_database(conn)
    config = KGFSConfig(
